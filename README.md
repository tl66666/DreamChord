...
- 新镜头自动继承上一镜头的背景和在场角色；新增人物不会清掉旧人物，只有明确“退场”才会生成隐藏事件。
- 创建选项、分支、跳转和汇合，并用剧情流程图检查整部故事。
- 播放器按每条分支独立计算舞台状态，角色和背景不会从另一条分支串入；汇合状态不一致时编辑器会提示重置。
- 场景实时预览、完整播放、发布和分享。
...
- 多对话创作 Agent：时间、问候等轻量问题本地即时回答；普通问题自然对话；明确任务才读取项目、分层记忆并调用受限工具。
- 模型返回 `patch: null`、代码块或附带说明时自动兼容；无法结构化时降级为安全只读答复，不再把简单问题判成失败。
- 未配置模型 API Key 时仍可问候、了解能力、概括项目、盘点素材、检查角色和剧情结构。
- 桌面、平板和手机宽度的响应式界面。
...
2. 进入故事工作台，选择章节和场景。
3. 新建场景。第一个场景带有效默认背景；后续场景会继承上一镜头的背景和仍在场角色。
4. 编辑镜头卡的说话人、表情、站位、对白或旁白。新人物选择“登场 / 更新”，旧人物保持在场或明确选择“退场”。
...
![DreamChord 自然对话与创作 Agent](docs/screenshots/agent-conversation-routing-1440.png)

Agent 不是一次性聊天框。你可以像普通聊天一样问“现在几点”“你能做什么”，也可以讨论一般知识或直接描述卡住的地方。每个项目可以新建、重命名、置顶、搜索和删除多个对话，不同对话分别负责续写、人物塑造、支线修补、剧情审查或素材准备。新对话默认不绑定章节，适合讨论整部作品；需要把修改写入故事时，再绑定具体章节。

输入区提供“了解项目、检查剧情、梳理角色、素材建议”四个起始任务。Agent 分三层工作：时间日期、问候和能力说明在本地即时完成；一般问题进入默认只读对话，可按需调用读取工具；只有明确的剧情修改或素材处理才进入受控行动和审批协议。
...
Agent 可以调用受限工具读取章节、检查剧情、提出结构化补丁，以及检查和准备立绘、CG、背景。普通对话接受模型直接返回的自然文本，不再强迫模型把每一句话包装成剧情 JSON；只读模式即使收到错误补丁也会丢弃补丁并保留文字结论。行动模式中的字段别名、不完整参数、`null` 字段或带说明 JSON 会在约束范围内规范化和修复。它不能调用任意 shell、文件系统或数据库，也不能绕过审核：
...
基础故事编辑、剧情体检、图片处理、备份恢复和播放器都不需要模型 API Key。没有配置 Key 时，Agent 自动使用 `dreamchord-local` 本地助手，可自然问候、说明能力、建议项目下一步、概括项目、盘点素材与角色、检查剧情图，并给出图片导入建议；这些操作不会访问外部模型。
...
测试覆盖剧情规则、舞台状态继承与分支隔离、保存冲突、Agent 对话路由和记忆隔离、上下文装配、工具调用、补丁应用/撤销、图片处理、备份恢复、编辑器历史、自动保存、响应式布局和核心界面流程。

## 相关文档

- [AI 与架构交接](docs/AI_HANDOFF.md)
- [创作 Agent 使用指南](docs/AGENT_GUIDE.md)
