...
- 多对话创作 Agent：普通问题自然回答，复杂任务读取项目上下文、分层记忆并调用受限工具。
...
![DreamChord 自然对话与创作 Agent](docs/screenshots/agent-capabilities-1440.png)

Agent 不是一次性聊天框。你可以像普通聊天一样说“你好”“你能做什么”或直接描述卡住的地方。每个项目可以新建、重命名、置顶、搜索和删除多个对话，不同对话分别负责续写、人物塑造、支线修补、剧情审查或素材准备。新对话默认不绑定章节，适合讨论整部作品；需要续写或修改剧情时，再绑定具体章节。

输入区提供“了解项目、检查剧情、梳理角色、素材建议”四个起始任务。Agent 分三层工作：即时对话在本地完成；项目理解会读取故事圣经、对话与记忆；只有需要执行时才进入工具协议。
...
Agent 可以调用受限工具读取章节、检查剧情、提出结构化补丁，以及检查和准备立绘、CG、背景。模型偶尔返回字段别名、不完整参数、`null` 字段或带说明的 JSON 时，运行器会在约束范围内规范化和修复；仍无法遵守协议但返回了可读文本时，只把文本作为对话答复保存，不调用工具、不写剧情。它不能调用任意 shell、文件系统或数据库，也不能绕过审核：
