...
- [x] 增加失败测试：`蒙太奇是什么` 返回定义和 DreamChord 操作建议。
- [x] 增加失败测试：`有哪些章节` 返回章节标题和节点数量。
- [x] 增加失败测试：无 Key 续写返回项目相关的本地结构草案，而不是纯配置提示。
- [x] 运行 `pnpm --filter dreamchord-server test -- localAssistant.test.ts`，确认断言因能力缺失失败。
- [x] 实现知识条目检索、章节摘要和项目相关续写方案。
- [x] 运行聚焦测试并确认通过。
...
- [x] 增加失败测试：Wikipedia 查询只访问 `zh.wikipedia.org`、返回摘要与来源、超时或失败返回 `null`。
- [x] 增加失败测试：`你记得什么` 使用传入的 memory 来源；`刚才聊了什么` 使用 conversation 来源。
- [x] 将本地助手改为异步，并注入可替换的公共知识查询函数。
- [x] 从 `runService` 传入隔离后的对话与记忆来源。
- [x] 运行 Agent 服务聚焦测试并确认通过。
...
- [x] 从 `745a360` 恢复展示页结构和视觉基线。
- [x] 替换真实桌面流程图截图。
- [x] 更新原 Agent 模块并新增零配置 Agent 桌面模块。
- [x] 不在主展示内容引用 `agent-390.png`、`editor-390.png` 或 `home-430.png`。
- [x] 更新就绪检查，要求新桌面 Agent 截图并禁止移动截图进入展示页。
...
- [x] 无 Key 发送创作术语、项目记忆、章节和一般事实问题。
- [x] 验证回复写入当前对话、没有剧情补丁、没有控制台错误。
- [x] 生成 1440×900 Agent 真实截图并放入展示页。
- [x] 检查展示页 1440×900 与 1024×768。
- [x] 更新能力边界，区分本地知识、公共知识查询和外部模型创作。
...
- [x] 运行 `pnpm test`。
- [x] 运行 `pnpm build`。
- [x] 运行 `pnpm test:readiness`。
- [x] 运行 `git diff --check`。
- [x] 冷启动并确认 API 与 Web 健康。
