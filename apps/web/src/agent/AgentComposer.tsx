...
export default function AgentComposer({ prompt, scope, disabled, hasProvider, hasChapter = true, onPromptChange, onScopeChange, onRun, onHealth, onOpenSettings }: {
  ...
  return (
    <div className="space-y-3 border-b border-slate-200 p-4">
      ...
      {!hasProvider && <p className="text-xs leading-5 text-amber-700">未配置模型时仍可读取项目、章节、角色、素材、对话和记忆，解释创作方法，并生成本地结构草案；开放式长文生成再配置外部模型。 <button type="button" onClick={onOpenSettings} className="font-semibold underline underline-offset-2">前往模型设置</button></p>}
