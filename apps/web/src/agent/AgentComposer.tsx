  ...
  return (
    <div className="space-y-3 border-b border-slate-200 p-4">
      <div className="grid grid-cols-2 gap-1 rounded-lg bg-slate-100 p-1" aria-label="Agent 作用域">
        {scopes.map((item) => <button key={item.value} type="button" onClick={() => onScopeChange(item.value)} className={`min-h-9 rounded-md px-2 text-xs font-medium transition ${scope === item.value ? 'bg-white text-slate-950 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>{item.label}</button>)}
      </div>
