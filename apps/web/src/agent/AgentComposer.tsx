...
export default function AgentComposer({ prompt, scope, disabled, hasProvider, hasChapter = true, targetId, targets, onPromptChange, onScopeChange, onTargetChange, onRun, onHealth, onOpenSettings }: {
  prompt: string; scope: AgentScope; disabled: boolean; hasProvider: boolean; hasChapter?: boolean
  targetId?: string | null; targets?: Array<{ id: string; label: string }>
  onPromptChange: (value: string) => void; onScopeChange: (scope: AgentScope) => void; onTargetChange?: (targetId: string) => void; onRun: () => void; onHealth: () => void; onOpenSettings: () => void
}) {
    ...
    <div className="space-y-3 border-b border-slate-200 p-4">
      ...
      <label className="block">
        ...
        <textarea aria-label="创作任务" value={prompt} onChange={(event) => onPromptChange(event.target.value)} onKeyDown={(event) => {
          ...
          submit()
        }} rows={5} placeholder="直接聊天，或描述你想检查、续写、修改的内容…" className="w-full resize-none rounded-lg border border-slate-200 bg-white px-3 py-2.5 text-sm leading-6 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-dream-500 focus:ring-2 focus:ring-dream-500/15" />
      </label>
      ...
      {!hasProvider && <p className="text-xs leading-5 text-amber-700">未配置模型时仍可读取项目、章节、角色、素材、对话和记忆，解释创作方法；对已选台词用“改成：新内容”可生成可审批的精确替换草案。开放式续写再配置外部模型。 <button type="button" onClick={onOpenSettings} className="font-semibold underline underline-offset-2">前往模型设置</button></p>}
