...
export async function importProject(input: unknown, userId: string, client: PrismaClient = prisma, storageRoot = process.env.UPLOAD_DIR || './uploads'): Promise<{ id: string; title: string }> {
  ...
  try {
    ...
    const result = await client.$transaction(async (tx) => {
      ...
      if (manifest.assets.length) await tx.asset.createMany({ data: manifest.assets.map((asset) => ({ id: assetIds.get(asset.id)!, projectId, name: asset.name, type: asset.type, url: resourceUrl(asset.resource, fileUrls), mimeType: decoded.get(asset.resource.kind === 'embedded' ? asset.resource.fileId : '')?.mimeType ?? asset.mimeType, width: asset.width, height: asset.height, hasAlpha: asset.hasAlpha, status: 'ready', metadata: JSON.stringify({ importedFromProject: manifest.project.id }) })) })
