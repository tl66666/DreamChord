...
export const projectManifestSchema = z.object({
  ...
  assets: z.array(z.object({ id: z.string().min(1), name: z.string().min(1).max(200), type: z.string().min(1).max(50), resource: resourceSchema, mimeType: z.string().nullable(), width: z.number().int().positive().nullable(), height: z.number().int().positive().nullable(), hasAlpha: z.boolean().nullable(), metadata: z.record(z.unknown()).default({}) }).strict()).max(10_000),
...
export async function exportProject(projectId: string, userId: string, client: PrismaClient = prisma, storageRoot = process.env.UPLOAD_DIR || './uploads'): Promise<ProjectManifest> {
  ...
  const assets = await Promise.all(exportAssets.map(async (asset) => ({ id: asset.id, name: asset.name, type: asset.type, resource: await embed(asset.url), mimeType: asset.mimeType, width: asset.width, height: asset.height, hasAlpha: asset.hasAlpha, metadata: parseObject(asset.metadata) })))
...
export async function importProject(input: unknown, userId: string, client: PrismaClient = prisma, storageRoot = process.env.UPLOAD_DIR || './uploads'): Promise<{ id: string; title: string }> {
  ...
  try {
    ...
    const result = await client.$transaction(async (tx) => {
      ...
      if (manifest.assets.length) await tx.asset.createMany({ data: manifest.assets.map((asset) => ({ id: assetIds.get(asset.id)!, ownerId: userId, projectId, name: asset.name, type: asset.type, url: resourceUrl(asset.resource, fileUrls), mimeType: decoded.get(asset.resource.kind === 'embedded' ? asset.resource.fileId : '')?.mimeType ?? asset.mimeType, width: asset.width, height: asset.height, hasAlpha: asset.hasAlpha, status: 'ready', metadata: JSON.stringify({ ...remapJson(asset.metadata, replacements) as Record<string, unknown>, importedFromProject: manifest.project.id }) })) })
